...
"> --}}
        ...
        @endif
        @if($content)
            <div class="mb-3 bg-gray-600 text-white p-4 rounded-xl">{!! Str::markdown((string) $content) !!}</div>
        @endif
        
        <div class="h-full flex items-center flex-col justify-center min-h-[80vh]" wire:show="!content">
    ...
    <div class="fixed bottom-4 w-full mx-auto flex justify-center">
        <form wire:submit.prevent="send" class="flex items-center justify-center">
            <input type="text" class="min-w-[700px] px-4 py-3 border-2 border-gray-400 h-full" placeholder="Write a message" wire:model="message" />
            <button class="min-w-[100px] h-full flex items-center justify-center bg-gray-200 border-2 border-gray-400"><i class="fa fa-send"></i></button>
        </form>
    </div>
